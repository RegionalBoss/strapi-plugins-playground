...
import { IDeploy } from "../../../server/content-types/deploy";
...
export function StageContainer({
  ...
  deploySetting: IDeploySetting;
  refetchTimer: number;
}) {
  const [deploys, setDeploys] = React.useState<IDeploy[]>([]);
          ...
          {deploys?.map((deploy) => (
            <li key={deploy.id}>
              <Typography as="h3" style={{ margin: "1rem 0" }}>
                <span>
                  {(deploy?.createdBy as IUser)?.firstname}{" "}
                  {(deploy?.createdBy as IUser)?.lastname}
                </span>
                <small>
                  {deploy.createdAt
